...
- 添加模拟器样例照片。
- 从系统相册导入单张或多张照片，并保存系统相册资产引用。
- 照片详情查看。
- 创建照片集，支持备注、模板和封面设置。
- 创建 tag 和分类。
- 给照片、照片集绑定 tag 和分类；给音频绑定 tag。
- 录音、暂停、继续、停止、保存。
- 导入本地音频文件。
- 保存录音或导入音频时绑定到单张照片或照片集。
- 音频列表播放。
- 搜索照片、照片集、音频、tag 和分类；已绑定 tag/分类也会命中对应内容。
...
当前阶段的开发边界是本地优先：暂不实现完整后端服务，暂不实现真实云端存储授权与上传。云端备份相关模型和模拟状态仅作为后续接口对接的占位，不作为当前本地 MVP 的验收条件。
...
4. 点击添加样例照片。
5. 在相册页从系统相册导入照片。
6. 选择照片创建照片集，并设置备注、模板和封面。
7. 在我的页创建 tag 和分类。
8. 进入照片详情或照片集详情绑定 tag 和分类。
9. 在录音页录制声音或导入本地音频，并绑定照片或照片集。
10. 在照片详情或照片集详情播放绑定音频。
11. 在搜索页搜索照片、照片集、tag、分类和音频，确认绑定 tag/分类可以命中内容。
